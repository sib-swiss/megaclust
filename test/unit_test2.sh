...
if [ ! -f "./test/shapes_in_noise.csv" ]; then
  echo "Error: test input data not found"
  echo
  exit 1
fi

if [ ! -f "./test/unit_test2.expected" ]; then
...
cut -d, -f1-4 ./test/shapes_in_noise.csv > $DIR/shapes.csv
...
./megaclust.sh -N $CPUs -C 3 -d $DIR/shapes -f 5.0 -l 100.0 -s 1.0  -n 200 -p 95.0 -U -v 1 > $DIR/shapes.megaclust.log

echo "done"
echo

CLUSTER0cnt=`grep ",0$" $DIR/shapes.clusters.sort | wc -l`
if [ $CLUSTER0cnt != 23869 ]; then
	echo "FAILED: Validation failed: unassigned=$CLUSTER0cnt but 43695 was expected."
fi

grep "^LOG: Cluster" $DIR/shapes.dclust | cut -c20- | sort -gr > $DIR/unit_test2.obtained

ERR=`diff ./test/unit_test2.expected $DIR/unit_test2.obtained | wc -l`
if [ $ERR == 0 ]; then
	echo "PASSED: Clustering Process was Successfull"
else
	echo "FAILED: Validation failed; see differences with expected results:"
	sdiff ./test/unit_test2.expected $DIR/unit_test2.obtained
fi
